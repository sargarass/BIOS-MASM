...
;;================================
w01:
        mov al, byte ptr [ebx + KB_INIT_TABLE + 0F0000h] ; команда в al
        mov dl, byte ptr [ebx + KB_INIT_TABLE + 1 + 0F0000h] ; закидываем порт
        out dx, al
        mov ah, byte ptr [ebx + KB_INIT_TABLE + 2 + 0F0000h] ; колво ответов
        lea ebx, [ebx + 3]
...
w11:
        in al, 060h
        mov dl, byte ptr [ebx + KB_INIT_TABLE + 0F0000h] ;; Ожидаемый ответ
        inc ebx
